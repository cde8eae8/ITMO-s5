...
test parser testSet expected expectedTail = do 
  it (show testSet) $ do 
    Just expectedTail `shouldBe` snd <$> parsingResult
    Just expected     `shouldBe` fst <$> parsingResult
...
tests = testSpec "" $ do
  describe "Parser tests" $ do
    describe "Functor" $ do
      ...
      test ((\x -> isSpace x) <$> element ' ') " bc" True "bc"
    ...
    describe "Lists" $ do
      ...
      testFail parseListOfLists "2, 1" 
      testFail parseListOfLists "" 
