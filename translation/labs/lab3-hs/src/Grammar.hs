...
happyExpList = Happy_Data_Array.listArray (0,77) ([20480,0,80,20480,0,0,0,0,24576,32768,8,128,0,0,0,16384,0,64,16384,0,0,16384,0,0,0,0,0,0,64,4352,0,0,16640,0,2,2048,16384,0,0,16384,0,80,20480,4,0,8192,0,512,20480,0,1104,0,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parse","SourceFile","Block","Expression","Expression1","OpExpression","FuncCallArgs","FuncCallArg","Type","PathIdentifier","if","else","identifier","'('","')'","'{'","'}'","'.'","','","';'","sign","%eof"]
...
action_14 (12) = happyGoto action_6
...
action_24 (8) = happyGoto action_18
...
happyReduction_4 (_ `HappyStk`
	...
	happyRest)
	 = HappyAbsSyn6
		 ("if " ++ happy_var_3 ++ " then { " ++ show happy_var_6 ++ "}"
	) `HappyStk` happyRest
...
happyReduction_5 (_ `HappyStk`
	...
	happyRest)
	 = HappyAbsSyn6
		 ("if " ++ happy_var_3 ++ " then { " ++ show happy_var_6 ++ "} else {" ++ show happy_var_10 ++ "}"
	) `HappyStk` happyRest
...
happyReduction_8 (_ `HappyStk`
	...
	happyRest)
	 = HappyAbsSyn7
		 (happy_var_1 ++ "[" ++ unwords happy_var_3 ++ "]"
	) `HappyStk` happyRest
...
happyReduction_9 ((HappyAbsSyn7  happy_var_6) `HappyStk`
	...
	happyRest)
	 = HappyAbsSyn7
		 (happy_var_1 ++ "[" ++ unwords happy_var_3 ++ "] ^." ++ happy_var_6
	) `HappyStk` happyRest
...
happyReduction_18 (HappyTerminal (Identifier happy_var_3))
	_
...
happyNewToken action sts stk (tk:tks) =
	...
	If -> cont 13;
	Else -> cont 14;
	Identifier happy_dollar_dollar -> cont 15;
	LBracket -> cont 16;
	RBracket -> cont 17;
	LBrace -> cont 18;
	RBrace -> cont 19;
	...
	Comma -> cont 21;
	DotComma -> cont 22;
	Sign happy_dollar_dollar -> cont 23;
	_ -> happyError' ((tk:tks), [])
	}
