...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      ...
      // Rules
      char dummy3[sizeof (std::vector<RawRule>)];
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_SourceFile = 11,                       // SourceFile
        S_Types = 12,                            // Types
        S_Type = 13,                             // Type
        S_Nonterminals = 14,                     // Nonterminals
        S_Nonterminal = 15,                      // Nonterminal
        S_StartNonterminal = 16,                 // StartNonterminal
        S_Tokens = 17,                           // Tokens
        S_Rule = 18,                             // Rule
        S_Rules = 19                             // Rules
      };
        ...
        switch (this->kind ())
    {
      case symbol_kind::S_Rule: // Rule
        value.move< RawRule > (std::move (that.value));
        break;
      ...
      case symbol_kind::S_Rules: // Rules
        value.move< std::vector<RawRule> > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, RawRule&& v)
        : Base (t)
        , value (std::move (v))
      {}
#else
      basic_symbol (typename Base::kind_type t, const RawRule& v)
        : Base (t)
        , value (v)
      {}
#endif
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, std::vector<RawRule>&& v)
        : Base (t)
        , value (std::move (v))
      {}
#else
      basic_symbol (typename Base::kind_type t, const std::vector<RawRule>& v)
        : Base (t)
        , value (v)
      {}
#endif
...
switch (yykind)
    {
      case symbol_kind::S_Rule: // Rule
        value.template destroy< RawRule > ();
        break;
      ...
      case symbol_kind::S_Rules: // Rules
        value.template destroy< std::vector<RawRule> > ();
        break;
    ...
    // Implementation of make_symbol for each symbol type.
      ...
      static
      symbol_type
      make_code (std::string v)
      {
        return symbol_type (token::code, std::move (v));
      }
#else
      static
      symbol_type
      make_code (const std::string& v)
      {
        return symbol_type (token::code, v);
      }
#endif
    ...
    , value ()
  {
    ...
    {
      case symbol_kind::S_Rule: // Rule
        value.copy< RawRule > (YY_MOVE (that.value));
        break;
      ...
      case symbol_kind::S_Rules: // Rules
        value.copy< std::vector<RawRule> > (YY_MOVE (that.value));
        break;
  ...
  parser::basic_symbol<Base>::move (basic_symbol& s)
  {
    ...
    {
      case symbol_kind::S_Rule: // Rule
        value.move< RawRule > (YY_MOVE (s.value));
        break;
      ...
      case symbol_kind::S_Rules: // Rules
        value.move< std::vector<RawRule> > (YY_MOVE (s.value));
        break;
